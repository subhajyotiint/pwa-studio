...
function getInstruction(rootDir, instructionType) {
    ...
    let instruction;
    try {
        instruction = require(instructionScriptPath);
    } catch (e) {
        throw new Error(
    ...
    try {
        return instruction({ fs: fse });
    } catch (e) {
        throw new Error(
            `Buildpack encountered an error running ${instructionScriptPath}. Original error: ${
                e.message
            }`
        );
    }
